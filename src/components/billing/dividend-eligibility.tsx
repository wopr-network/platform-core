      ...
      <Card>
        <CardContent className="p-4 space-y-3">
          <div className="flex items-center gap-2">
            <ClockIcon
              className={cn("size-4 shrink-0", isExpiringSoon ? "text-amber-500" : "text-terminal")}
            />
            <p className="text-sm font-medium">
              You&apos;re in the pool for{" "}
              <span
                className={cn("font-bold", isExpiringSoon ? "text-amber-500" : "text-terminal")}
              >
                {daysRemaining}
              </span>{" "}
          ...
          />
          {isExpiringSoon && (
            <p className="text-xs text-amber-500">Buy credits to reset your 7-day window</p>
          )}
        </CardContent>
      </Card>
    </motion.div>
